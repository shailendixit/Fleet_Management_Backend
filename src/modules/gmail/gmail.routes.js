...
const { processUnreadGmailMessages,processSpecificMessage } = require('../../automation/emailautomation');
...
router.post('/push', async (req, res) => {
    try {
        // 1. Handle Microsoft Graph validation token
        ...
        // 2. Acknowledge the notification immediately to prevent retries from Microsoft
        res.status(200).send('OK');
        ...
        if (notifications.length > 0) {
            ...
            for (const n of notifications) {
                ...
                if (msgId) {
                    console.log(`Targeted push: Processing message ${msgId}`);
                    // Use a specific fetcher that skips the "search index"
                    processSpecificMessage(msgId).catch(err => 
                        console.error(`Error in targeted process for ${msgId}:`, err)
                    );
                }
            }
        } else {
            // Fallback for empty notifications or other sources
            console.log('Generic notification received — starting async unread poll');
            processUnreadGmailMessages().catch(err => 
                console.error('Async processUnreadGmailMessages error:', err)
            );
        }
    } catch (e) {
        console.error('Error handling mail push:', e);
